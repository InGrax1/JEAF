...
### Corregido
- **"Error inesperado" al registrar ingresos/gastos desde el celular en pruebas locales**: `crypto.randomUUID()` exige un contexto seguro (HTTPS o `localhost`); al probar con `npm run dev -- --host` y entrar desde el teléfono por la IP de red (`http://192.168.x.x:5173`, sin HTTPS), no existe y tronaba antes de llegar al backend, cayendo en el mensaje genérico. El backend solo exige que `idempotencyKey` sea un string de 8-191 caracteres (no un UUID con formato específico), así que `AccionesRapidasTransaccion.tsx` ahora degrada a `crypto.getRandomValues()` (disponible en cualquier contexto) y, como último recurso, a `Date.now()` + `Math.random()`. Verificado deshabilitando `crypto.randomUUID` en el navegador para simular el mismo escenario.

### Agregado
- **Registrar ingresos/gastos directo desde la web** (`components/AccionesRapidasTransaccion.tsx`), sin depender del Atajo de iOS: el backend ya soportaba ambos orígenes en `POST /transacciones` (`authDual` — panel web y Atajo comparten el mismo endpoint, spec 3.1), solo faltaba la UI. Dos botones flotantes ("Ingreso"/"Gasto", fijos abajo a la derecha, con el área segura de iOS) viven en `Layout.tsx` fuera del `<Outlet />`, así que están disponibles en cualquier página del panel — solo para el Tesorero (`super_admin`), igual que el resto de acciones administrativas (cancelar, conciliar, etc.). Cada botón abre el modal ya con el tipo fijo y las categorías filtradas a ese tipo; usa una `idempotencyKey` nueva (`crypto.randomUUID()`) por envío para evitar duplicados por doble clic. Al guardar, dispara un evento (`jeaf:transaccion-creada`) que el Dashboard y Transacciones escuchan para refrescarse solos si están montados — el Dashboard lo hace en silencio, sin volver a mostrar la pantalla de carga completa.
- **Animaciones de UI con Motion** (ya en el proyecto desde la pantalla de carga — se descartó agregar Anime.js para no duplicar librería de animación):
  - **Modal** (`ui.tsx`): antes se cerraba de golpe (sin animación de salida); ahora `AnimatePresence` vive dentro del propio componente para que apertura y cierre animen (fondo con fade, tarjeta con resorte de escala/posición). Al ser compartido, todas las páginas con modal (Transacciones, Categorías, Usuarios, API Keys) lo heredan automáticamente.
  - **Indicador de navegación activo** (`Layout.tsx`): el resaltado de la barra lateral ya no aparece/desaparece de golpe en cada ítem — es un solo elemento (`layoutId`) que Motion desliza suavemente entre rutas al navegar.
  - **Mensajes de error** (`MensajeError` en `ui.tsx`): entrada con fade + deslizamiento sutil en vez de aparecer abruptamente.
  - **Tarjetas de login/recuperar contraseña**: la entrada con resorte reemplaza el keyframe CSS `animate-modal-in` (eliminado de `index.css` por quedar sin uso).
  - **Filas de tablas** (Transacciones, Categorías, Usuarios, API Keys): entrada escalonada (fade + deslizamiento, con un pequeño retraso creciente por fila) en vez de aparecer todas de golpe.
  - **Tarjetas KPI del Dashboard**: mismo patrón de entrada escalonada.
