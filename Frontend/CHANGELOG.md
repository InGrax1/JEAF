...
- **Corregido el destello negro al arrancar la PWA en iOS**: un `<style>` inline en `index.html` pinta el fondo (`#f7f9fb`) desde el primer frame, antes de que cargue el CSS/JS — en standalone iOS no hay imagen de arranque y el fondo por defecto es negro.
